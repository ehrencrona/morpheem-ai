	...
	await Promise.all(
		batch.map(async (wordString) => {
			try {
				...
				if (word.unit == null) {
					await setWordUnit(units[Math.floor(Math.random() * units.length)].id, word.id, language);
				}
			} catch (e) {
